...
def message_for_incoming_request(bid_data_job_id):
	bid_data = job.Job.fetch(bid_data_job_id).result
	msg = (f"На вашу заявку пришел запрос, вы готовы ее исполнить?\n"
		  f"Если да, мы направим ваши контакты пользователю\n"
		  f"Заявка:\n {bid_string_overview(bid_data)}")
	return msg
...
def bid_string_overview(bid_data):
	NEED_VAL = int(bid_data["NEED_VAL"])
	HAS_VAL = int(bid_data["HAS_VAL"])
	...
	msg = (f"Находится в {LOC_MAIN_ALIAS}\n"
		   f"{HAS_VAL:,} {HAS_CUR} "
		   f"{trans_CUR_LOC(HAS_LOC)}\n"
		   f"Вам нужно {NEED_VAL:,} {NEED_CUR} "
...
def bid_string_without_values(bid_data):
	...
	HAS_LOC = bid_data["HAS_LOC"]
	LOC_MAIN_ALIAS = bid_data["LOC_MAIN_ALIAS"]
	msg = (f"Находится в х км\n"
		   f"{HAS_VAL:,} {HAS_CUR} "
		   f"{trans_CUR_LOC(HAS_LOC)}\n"
		   f"Просят {NEED_VAL:,} {NEED_CUR} "
		   f"{trans_CUR_LOC(NEED_LOC)}\n"
